...
def open_database(path='bank.db'):
    ...
    if new:
        ...
        add_payment(db, 'yuwen', 'man1', 5000, 'Registration for PyCon')
        add_payment(db, 'yuwen', 'man2', 5000, 'Payment for writing that code')
        add_payment(db, 'man1', 'yuwen', 100, 'Gas money-thanks for the ride!')
        db.commit()
    return db
